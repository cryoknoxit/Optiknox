...
namespace OptiKnoxAPI.Repositories
{
    public class AzureStorage: IAzureStorage
    {
        ...
        public async Task<BlobDto> DownloadAsync(string blobFilename)
        {
            ...
            try
            {
                ...
                if (await file.ExistsAsync())
                {
                    ...
                    string blobUrl = file.Uri.ToString();
                    // Create new BlobDto with blob data from variables
                    return new BlobDto { Content = blobContent, Name = name, ContentType = contentType, Uri = blobUrl };
                }
            }
            catch (RequestFailedException ex)
                when (ex.ErrorCode == BlobErrorCode.BlobNotFound)
        ...
        public async Task<BlobResponseDto> UploadAsync(IFormFile blob,string FileName,string FileID)
        {
            ...
            try
            {
                ...
                response.Status = $"File {FileID} Uploaded Successfully _Responce" + blob.FileName;
               ...
               when (ex.ErrorCode == BlobErrorCode.BlobAlreadyExists)
            {
                ...
                response.Status = $"File with name {blob.FileName} already exists. Please use another name to store your file._Responce" + blob.FileName;
                response.Error = true;
                return response;
            }
